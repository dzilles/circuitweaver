...
- [implemented] `ARCH-004` The compiler shall not skip auto-layout solely because any schematic element is present; it shall validate whether the schematic layer is complete for the requested operation.
- [implemented] `ARCH-005` Pipeline stages shall return structured result objects instead of relying on exceptions and side effects for normal failure reporting.
...
- [implemented] `ARCH-006` The compilation pipeline shall expose intermediate artifacts for tests without requiring debug files on disk.
...
- [implemented] `ARCH-022` Pipeline stages shall accept and return `CircuitProject` or dedicated stage result types instead of passing a raw `list[CircuitElement]` through all stages.
...
- [implemented] `VALP-001` Validation shall support named profiles.
- [implemented] `VALP-002` The `source` validation profile shall validate logic-only Circuit JSON for schema, source IDs, source references, trace connectivity, and source port completeness.
- [implemented] `VALP-003` The `schematic` validation profile shall validate schematic elements, schematic references, geometry, labels, ports, and no-connects.
...
- [implemented] `DOCTOR-001` The CLI shall provide `circuitweaver doctor`.
- [implemented] `DOCTOR-002` `circuitweaver doctor` shall check Python package version and importability.
- [implemented] `DOCTOR-003` `circuitweaver doctor` shall check that Node.js is available.
- [implemented] `DOCTOR-004` `circuitweaver doctor` shall check that `elkjs` is resolvable from the current runtime.
- [implemented] `DOCTOR-005` `circuitweaver doctor` shall check detected KiCad symbol, footprint, 3D model, and template paths.
- [implemented] `DOCTOR-006` `circuitweaver doctor` shall check that `kicad-cli` is available.
- [implemented] `DOCTOR-007` `circuitweaver doctor` shall check that packaged MCP resources can be read.
- [implemented] `DOCTOR-008` `circuitweaver doctor` shall support machine-readable JSON output.

## HTTP MCP Transport

- [implemented] `ARCH-060` HTTP MCP support shall use an official MCP SDK Streamable HTTP transport or another standards-compliant MCP HTTP transport.
...
- [implemented] `ARCH-061` Hosted HTTP MCP support shall support authentication hooks.
- [implemented] `ARCH-062` Hosted HTTP MCP support shall support request limits or quota hooks.
...
- [implemented] `TEST-001` KiCad S-expression generation shall support deterministic UUID generation for tests.
- [implemented] `TEST-002` Compile pipeline tests shall be able to inject symbol lookup, router, and ERC dependencies without monkeypatching module imports.
- [implemented] `TEST-003` Requirement-to-test traceability shall be machine-checkable for all requirement files, not only CLI/MCP requirements.
